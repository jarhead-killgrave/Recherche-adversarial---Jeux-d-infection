...
class Image:
    def __init__(self):
        ...
        self.representation = {
            ...
            0: pygame.image.load("./images/case-rouge-1.png"),
            1: pygame.image.load("./images/case-bleu-1.png"),
        }
    ...
    def dessiner_grille(self, plateau: np.ndarray, ecran: Union[Surface, SurfaceType]):
        coords = np.array(np.meshgrid(
            np.arange(7), np.arange(7))).T.reshape(-1, 2)
        for coord in coords:
            val = plateau[tuple(coord)]
            if val in self.representation:
                ecran.blit(
                    self.representation[val], (coord[1] * self.taille, coord[0] * self.taille))
